...
建议第一个新增接口选择 `daily_basic`。

原因：

- 它是量化研究中非常常用的数据，包含估值、市值、换手率等指标。
- 它与现有 `daily_kline` 同属交易日增量型，开发风险低。
- 它能验证通用日分区同步和本地查询抽象是否成立。
...
当 `daily_basic` 的端到端流程跑通后，再按同一模式扩展 `stk_limit`、`limit_list_d` 和 `moneyflow`。
