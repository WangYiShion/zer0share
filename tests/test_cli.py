...
def test_sync_stk_limit_accepts_date_range():
    runner = CliRunner()
    pipeline = MagicMock()
    ...
    pipeline.sync_stk_limit.assert_called_once_with(
...
def test_sync_daily_basic_accepts_date_range():
    runner = CliRunner()
    pipeline = MagicMock()
    pipeline.__enter__.return_value = pipeline
    pipeline.__exit__.return_value = False
    ...
    assert result.exit_code == 0
    pipeline.sync_daily_basic.assert_called_once_with(
        start_date=date(2016, 1, 1),
        end_date=date(2016, 1, 31),
    )
